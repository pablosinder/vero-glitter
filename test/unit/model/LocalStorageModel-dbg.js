...
sap.ui.define([
	"com/veroglitter/model/LocalStorageModel",
	"sap/ui/util/Storage"
], (LocalStorageModel, Storage) => {
	"use strict";

	QUnit.module("LocalStorageModel - Defaults");

	QUnit.test("Should initialize the local storage model with its defaults", (assert) => {
		// Act
		const oLocalStorageModel = new LocalStorageModel();
	...
	QUnit.test("Should initialize the local storage model properly", function (assert) {
